...
import (
	...
	samplecontroller "batata/feijao/pkg/apis/samplecontroller/v1alpha1"
	"batata/feijao/pkg/generated/clientset/versioned/fake"
	informers "batata/feijao/pkg/generated/informers/externalversions"
)
...
func (f *fixture) newController(ctx context.Context) (*Controller, informers.SharedInformerFactory, kubeinformers.SharedInformerFactory) {
	...
	c := NewController(ctx, f.kubeclient, f.client,
		k8sI.Apps().V1().Deployments(), i.Teste().V1alpha1().Foos())
	...
	for _, f := range f.fooLister {
		i.Teste().V1alpha1().Foos().Informer().GetIndexer().Add(f)
	}
